...
export default function ResultCard({ data }: { data: SSLData }) {
  ...
  const getExpiryYear = (dateStr?: string) => (!dateStr || dateStr === "Not Available") ? "N/A" : new Date(dateStr).getFullYear();
  ...
  return (
    <div className="mt-8 md:mt-12 w-full animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="rounded-[2rem] md:rounded-[2.5rem] border-2 border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 md:p-10 shadow-xl">
        <div className="flex flex-col md:flex-row items-center gap-6 md:gap-10 text-center md:text-left">
          <div className={`flex h-20 w-20 md:h-28 md:w-28 shrink-0 items-center justify-center rounded-[1.8rem] md:rounded-[2.2rem] ${
            ...
            {isSecure ? <ShieldCheck className="text-white w-10 h-10 md:w-16 md:h-16" /> : <ShieldAlert className="text-white w-10 h-10 md:w-16 md:h-16" />}
          </div>
          <div>
            ...
            <p className="mt-2 text-sm md:text-xl font-medium text-slate-500 dark:text-slate-400">
              Verified security diagnostics for this domain.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 md:mt-6 grid grid-cols-1 gap-4 md:gap-6 lg:grid-cols-3">
        <BentoTile label="Security Level" value={data.type} icon={<Fingerprint className="text-indigo-500 w-6 h-6 md:w-8 md:h-8" />} description="Encryption Tier" />
        
        {/* break-words ensures 'Google Trust Services' etc. wrap naturally */}
        <BentoTile label="Authority" value={data.issuer} icon={<Building2 className="text-blue-500 w-6 h-6 md:w-8 md:h-8" />} description="Verified Issuer" />
        
        <BentoTile label="Domain" value={`Expires ${getExpiryYear(data.domainExpiry)}`} icon={<Globe className="text-amber-500 w-6 h-6 md:w-8 md:h-8" />} description="Registry Ownership" />
        ...
        <div className="lg:col-span-3 rounded-[2rem] md:rounded-[2.5rem] border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 md:p-10">
          <div className="mb-6 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] md:text-xs text-slate-400 font-bold uppercase tracking-widest italic">Valid until {new Date(data.expiryDate).toLocaleDateString()}</p>
              </div>
            </div>
          ...
          <div className="h-4 w-full rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
            <div className="h-full bg-emerald-500 transition-all duration-1000" style={{ width: `${Math.max(5, (data.daysLeft/365)*100)}%` }} />
          </div>
        </div>
      </div>
    </div>
...
function BentoTile({ label, value, icon, description }: any) {
  return (
    <div className="flex flex-col rounded-[1.8rem] md:rounded-[2.2rem] border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 md:p-8 transition-all hover:scale-[1.03]">
      <div className="mb-5 flex h-12 w-12 md:h-16 md:w-16 items-center justify-center rounded-2xl bg-slate-50 dark:bg-slate-800">{icon}</div>
      <p className="text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">{label}</p>
      <h4 className="text-lg md:text-2xl font-black text-slate-800 dark:text-white leading-tight break-words">{value}</h4>
