...
export async function POST(req: Request) {
  try {
    ...
    const [sslData, rdapData] = await Promise.all([
      performSSLCheck(host),
      fetch(`https://rdap.org/domain/${host}`).then(res => res.json()).catch(() => ({}))
    ]);

    // Extracting domain expiry from RDAP events
    const expiryEvent = rdapData.events?.find((e: any) => e.eventAction === 'expiration');
    const domainExpiry = expiryEvent ? expiryEvent.eventDate : "Not Available";

    if (sslData.error) {
      return NextResponse.json({ error: sslData.error }, { status: 400 });
    }
  ...
  return new Promise((resolve) => {
    const socket = tls.connect({ 
      ...
      if (!cert || Object.keys(cert).length === 0) {
        socket.destroy();
        resolve({ error: "No certificate found on Port 443." });
        return;
      }
    ...
    socket.on('timeout', () => {
      socket.destroy();
      resolve({ error: "Connection timed out. Check the domain spelling." });
    });

    socket.on('error', (err: any) => {
      // Map technical codes to human-friendly messages
      let msg = "Connection failed.";
      if (err.code === 'ENOTFOUND') msg = "Domain not found. Check for typos like .com.com";
      else if (err.code === 'ECONNREFUSED') msg = "Connection refused by the server.";
      resolve({ error: msg });
    });
  });
}